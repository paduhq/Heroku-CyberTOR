...
function bot(torrent, bot) {
  ...
  bot.onText(downloadRegex, (msg, match) => {
    ...
    const onDriveUpload = (torr, url) => bot.sendMessage(from, `${torr.name} uploaded to Torrent Padu's Google Drive\n${url}\n\nWeb Link: ${torr.downloadLink}`);
    const onDriveUploadStart = torr => bot.sendMessage(from, `Uploading ${torr.name} to Torrent Padu's Google Drive`);
